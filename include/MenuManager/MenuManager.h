...
typedef struct RGBColor {
    unsigned short red;     /* 0x0000 = black, 0xFFFF = full red */
    unsigned short green;   /* 0x0000 = black, 0xFFFF = full green */
    unsigned short blue;    /* 0x0000 = black, 0xFFFF = full blue */
} RGBColor;
...
enum {
    noMark              = '\0',         /* No mark symbol for MarkItem */

    /* Special command characters */
    commandMark         = 0x11,         /* Command key symbol */
    checkMark           = 0x12,         /* Check mark character */
    diamondMark         = 0x13,         /* Diamond mark character */
    appleMark           = 0x14,         /* Apple logo mark */

    /* Menu definition procedure messages */
    mDrawMsg            = 0,            /* Draw the menu */
    mChooseMsg          = 1,            /* Choose from menu */
    mSizeMsg            = 2,            /* Calculate menu size */
    mPopUpMsg           = 3,            /* Popup menu positioning */
    mDrawItemMsg        = 4,            /* Draw menu item */
    mCalcItemMsg        = 5,            /* Calculate item dimensions */

    /* Standard menu definition procedures */
    textMenuProc        = 0,            /* Standard text menu MDEF */

    /* Hierarchical menu support */
    hMenuCmd            = 27,           /* itemCmd == 0x001B ==> hierarchical menu */
    hierMenu            = -1,           /* Hierarchical menu for InsertMenu call */

    /* Menu color table constants */
    mctAllItems         = -98,          /* Search for all items for given ID */
    mctLastIDIndic      = -99           /* Last color table entry has this in ID field */
};

/* System menu ID ranges */
enum {
    kLoSystemMenuRange  = 0xB000,       /* -20480: Lowest system menu ID */
    kHiSystemMenuRange  = 0xBFFF,       /* -16385: Highest system menu ID */
    kApplicationMenuID  = 0xBF97,       /* -16489: Application menu ID */
    kHelpMenuID         = 0xBF96,       /* -16490: Help menu ID */
    kScriptMenuID       = 0xBF95        /* -16491: Keyboard/script menu ID */
};
...
enum {
    ...
    outline             = 8,            /* Outlined text */
    shadow              = 16,           /* Shadow text */
    condense            = 32,           /* Condensed text */
    extend              = 64            /* Extended text */
};
...
/* Menu information structure - core menu data */
struct MenuInfo {
    short menuID;                       /* Menu identifier */
    short menuWidth;                    /* Width in pixels */
    short menuHeight;                   /* Height in pixels */
    Handle menuProc;                    /* Menu definition procedure handle */
    long enableFlags;                   /* Enable flags for items 0-31 */
    Str255 menuData;                    /* Menu title + item data */
};

/* Menu color entry for customizing menu appearance */
typedef struct MCEntry {
    short mctID;                        /* Menu ID (0 = menu bar) */
    short mctItem;                      /* Menu item (0 = title) */
...
typedef void (*MenuDefProcPtr)(short message, MenuHandle theMenu,
...
typedef struct MenuManagerState {
    ...
    short currentMenuBar;               /* Current menu bar resource ID */
    short hiliteMenu;                   /* Currently highlighted menu ID */
    short menuBarHeight;                /* Height of menu bar in pixels */
    Boolean menuBarVisible;             /* TRUE if menu bar is visible */
    Boolean menuBarInvalid;             /* TRUE if menu bar needs redrawing */
    MCTableHandle menuColorTable;       /* Menu color table */
    short menuFlash;                    /* Flash count for menu feedback */
    long lastMenuChoice;                /* Last menu selection result */

    /* Menu tracking state */
    Boolean trackingMenu;               /* TRUE during menu tracking */
    MenuHandle currentMenu;             /* Menu being tracked */
    short currentItem;                  /* Item being tracked */
    Point lastMenuPt;                   /* Last mouse point during tracking */

    /* Platform-specific data */
    void* platformData;                 /* Platform menu system integration */
    Boolean initialized;                /* TRUE when Menu Manager is ready */
} MenuManagerState;
