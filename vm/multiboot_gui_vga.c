...
static void init_vga_mode13(void) {
    /* Set VGA mode 13h using BIOS interrupt would be here,
       but in protected mode we need to do it manually */

    /* For simplicity in a multiboot kernel, assume VGA is already
       in a graphics-capable state and just set up the palette */
