...
static uint32_t gScrapStats[4] = {0}; /* put, get, conversion, error counts */

/* Internal function prototypes */
static OSErr AllocateFormatTable(int16_t maxFormats);
static void FreeFormatTable(void);
static OSErr AddFormatEntry(ResType type, int32_t size, int32_t offset);
...
OSErr InitScrapManager(void)
{
    ...
    gScrapStuff.lastModified = (uint32_t)time(NULL);
...
{
    int32_t size = GetScrapSize(SCRAP_TYPE_TEXT);
    return (size > 32767) ? 32767 : (int16_t)size;
}
