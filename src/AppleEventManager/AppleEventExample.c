...
static OSErr HandleOpenDocuments(const AppleEvent* theAppleEvent, AppleEvent* reply, int32_t handlerRefcon) {
    ...
    for (int32_t i = 1; i <= documentCount; i++) {
        AEDesc fileDesc;
        AEKeyword keyword;
...
static OSErr HandleQuitApplication(const AppleEvent* theAppleEvent, AppleEvent* reply, int32_t handlerRefcon) {
...
static OSErr HandleGetInfo(const AppleEvent* theAppleEvent, AppleEvent* reply, int32_t handlerRefcon) {
...
static void DemonstrateProcessTargeting(void) {
    ...
    int32_t displayCount = (processCount < 5) ? processCount : 5;
