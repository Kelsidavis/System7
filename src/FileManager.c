...
OSErr FSSetFPos(FileRefNum refNum, uint16_t posMode, int32_t posOffset)
{
    ...
    switch (posMode) {
        case fsFromStart:
            ...
            newPos = (uint32_t)posOffset;
            break;

        case fsFromLEOF:
            if (posOffset > 0 || (uint32_t)(-posOffset) > fcb->fcbEOF) {
                FS_UnlockFCB(fcb);
                return posErr;
            }
        ...
        case fsFromMark:
            if ((posOffset < 0 && (uint32_t)(-posOffset) > fcb->fcbCrPs) ||
...
OSErr FSGetVInfo(VolumeRefNum vRefNum, StringPtr volName, uint16_t* vRefNumOut, uint32_t* freeBytes)
{
    VCB* vcb;
    OSErr err;
    ...
    if (freeBytes) {
        *freeBytes = (uint32_t)vcb->vcbFreeBks * vcb->vcbAlBlkSiz;
    }
...
OSErr PBReadSync(ParmBlkPtr paramBlock)
{
    ...
    if (!fcb) {
        ...
        /* Read the data */
        err = IO_ReadFork(fcb, fcb->fcbCrPs, paramBlock->u.ioParam.ioReqCount,
                         paramBlock->u.ioParam.ioBuffer, &actualCount);

        paramBlock->u.ioParam.ioActCount = actualCount;
        paramBlock->u.ioParam.ioPosOffset = fcb->fcbCrPs;
...
OSErr PBWriteSync(ParmBlkPtr paramBlock)
{
    ...
    if (!fcb) {
        ...
        if (!(fcb->fcbFlags & FCB_WRITE_PERM)) {
            ...
            /* Write the data */
            err = IO_WriteFork(fcb, fcb->fcbCrPs, paramBlock->u.ioParam.ioReqCount,
                             paramBlock->u.ioParam.ioBuffer, &actualCount);
...
OSErr PBHOpenDFSync(ParmBlkPtr paramBlock)
{
    ...
    /* Open data fork */
    err = FCB_Open(vcb, paramBlock->u.fileParam.ioDirID,
...
OSErr PBHCreateSync(ParmBlkPtr paramBlock)
{
    ...
    err = Cat_Create(vcb, paramBlock->u.fileParam.ioDirID,
...
OSErr PBHDeleteSync(ParmBlkPtr paramBlock)
{
    ...
    /* Delete from catalog */
    err = Cat_Delete(vcb, paramBlock->u.fileParam.ioDirID,
                    paramBlock->ioNamePtr);
...
OSErr PBHRenameSync(ParmBlkPtr paramBlock)
{
    ...
    /* Rename in catalog */
    err = Cat_Rename(vcb, paramBlock->u.fileParam.ioDirID,
                     paramBlock->ioNamePtr,
                     (const uint8_t*)paramBlock->u.ioParam.ioMisc);
...
uint32_t DateTime_FromUnix(time_t unixTime)
{
    ...
    return (uint32_t)(unixTime + MAC_EPOCH_OFFSET);
}
