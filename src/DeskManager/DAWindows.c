...
typedef struct DAWindow {
    ...
    int16_t         windowType;        /* Window type */
    uint16_t        attributes;        /* Window attributes */
    bool            visible;           /* Window visibility */
    bool            active;            /* Window active state */
    bool            needsUpdate;       /* Needs redraw */
    void            *userData;         /* User data */

    /* Event handling */
    bool            trackingMouse;     /* Tracking mouse */
...
static bool DAWindow_PointInWindow(DAWindow *window, Point point);
...
int DA_CreateWindow(DeskAccessory *da, const DAWindowAttr *attr)
{
    ...
    window->contentRect.top += 20;  /* Title bar height */
    window->contentRect.left += 1;  /* Left border */
    window->contentRect.right -= 1; /* Right border */
    window->contentRect.bottom -= 1; /* Bottom border */
 ...
 * Move DA window
 */
void DAWindow_Move(DeskAccessory *da, int16_t h, int16_t v)
{
    if (!da || !da->window) {
        return;
    }
    ...
    DAWindow *window = (DAWindow *)da->window;
    int16_t deltaH = h - window->bounds.left;
    int16_t deltaV = v - window->bounds.top;
    ...
    /* Update content rect */
    window->contentRect.left += deltaH;
    window->contentRect.top += deltaV;
    window->contentRect.right += deltaH;
    window->contentRect.bottom += deltaV;
 ...
 * Resize DA window
 */
void DAWindow_Resize(DeskAccessory *da, int16_t width, int16_t height)
{
    if (!da || !da->window) {
        return;
    }
    ...
    window->contentRect.top += 20;  /* Title bar */
    window->contentRect.left += 1;  /* Border */
    window->contentRect.right -= 1;
    window->contentRect.bottom -= 1;
 ...
 * Activate DA window
 */
void DAWindow_Activate(DeskAccessory *da, bool active)
{
    if (!da || !da->window) {
        return;
    }
 ...
 */
bool DAWindow_IsActive(DeskAccessory *da)
{
    if (!da || !da->window) {
        return false;
    }
...
int DAWindow_HandleMouseDown(DeskAccessory *da, const EventRecord *event)
{
    ...
    localPoint.h -= window->contentRect.left;
    localPoint.v -= window->contentRect.top;
 ...
 * Set window position
 */
void DAWindow_SetPosition(DeskAccessory *da, int16_t h, int16_t v)
{
 ...
 * Get window position
 */
void DAWindow_GetPosition(DeskAccessory *da, int16_t *h, int16_t *v)
{
    if (!da || !da->window || !h || !v) {
        return;
    }
 ...
 * Set window size
 */
void DAWindow_SetSize(DeskAccessory *da, int16_t width, int16_t height)
{
 ...
 * Get window size
 */
void DAWindow_GetSize(DeskAccessory *da, int16_t *width, int16_t *height)
{
