...
typedef struct PopupData {
    ...
    int16_t selectedItem;           /* Currently selected item */
    int16_t variation;              /* Popup variation flags */

    /* Layout */
    Rect labelRect;                 /* Label area rectangle */
    Rect menuRect;                  /* Menu area rectangle */
    Rect triangleRect;              /* Triangle area rectangle */
    int16_t titleWidth;             /* Width of title text */

    /* Style flags */
    bool fixedWidth;                /* Fixed width popup */
    bool useWFont;                  /* Use window font */
    bool useAddResMenu;             /* Use AddResMenu */
    bool hasTitle;                  /* Has title label */

    /* Title style */
    int16_t titleStyle;             /* Title text style */
    int16_t titleJust;              /* Title justification */
    RGBColor titleColor;            /* Title text color */

    /* Visual state */
    bool menuDown;                  /* Menu is currently down */
    bool tracking;                  /* Currently tracking */

    /* Platform support */
    bool useNativePopup;            /* Use native popup control */
    Handle nativeControl;           /* Native control handle */

} PopupData;

/* Internal function prototypes */
static void InitializePopupData(ControlHandle popup, int16_t varCode);
...
static int16_t CalcPopupPart(ControlHandle popup, Point pt);
...
static void UpdatePopupSelection(ControlHandle popup, int16_t item);
static OSErr LoadPopupMenuItems(ControlHandle popup);
static void GetPopupItemText(ControlHandle popup, int16_t item, Str255 text);
 ...
 */
int32_t PopupMenuCDEF(int16_t varCode, ControlHandle theControl,
...
ControlHandle NewPopupControl(WindowPtr window, const Rect *bounds,
                             ...
                             int16_t menuID, int16_t variation,
                             int32_t refCon) {
...
static void CalculatePopupRects(ControlHandle popup) {
    ...
    if (popupData->hasTitle && popupData->titleWidth > 0) {
        ...
        popupData->menuRect.right -= POPUP_ARROW_WIDTH;

        popupData->triangleRect = bounds;
        popupData->triangleRect.left = popupData->triangleRect.right - POPUP_ARROW_WIDTH;
    } else {
        /* Popup without title */
        popupData->labelRect.left = popupData->labelRect.right = bounds.left;
...
static void DrawPopupLabel(ControlHandle popup) {
    ...
    case popupTitleLeftJust:
        h = popupData->labelRect.left + POPUP_MARGIN;
        break;
    case popupTitleCenterJust:
        h = popupData->labelRect.left +
            (popupData->labelRect.right - popupData->labelRect.left -
             StringWidth((*popup)->contrlTitle)) / 2;
        break;
    case popupTitleRightJust:
        h = popupData->labelRect.right - StringWidth((*popup)->contrlTitle) - POPUP_MARGIN;
        break;
    default:
        h = popupData->labelRect.left + POPUP_MARGIN;
        break;
    ...
    v = popupData->labelRect.top +
        (popupData->labelRect.bottom - popupData->labelRect.top + fontInfo.ascent) / 2;
...
static void DrawPopupMenu(ControlHandle popup) {
    ...
    h = popupData->menuRect.left + POPUP_MARGIN;
    v = popupData->menuRect.top +
        (popupData->menuRect.bottom - popupData->menuRect.top + fontInfo.ascent) / 2;
...
static void DrawPopupTriangle(ControlHandle popup) {
    ...
    centerH = (popupData->triangleRect.left + popupData->triangleRect.right) / 2;
    centerV = (popupData->triangleRect.top + popupData->triangleRect.bottom) / 2;
