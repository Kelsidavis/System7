...
typedef enum {
    ...
    kWindowStateMinimized = 1,
    kWindowStateMaximized = 2,
    kWindowStateHidden = 3,
    kWindowStateClosing = 4
} WindowState;
...
typedef struct WindowExtension {
    WindowState state;              /* Current window state */
    WindowUpdateFlags updateFlags;  /* Pending updates */
    unsigned long lastUpdateTime;   /* Last update timestamp */
    void* platformWindow;           /* Platform-specific window handle */
    Boolean needsRecalc;            /* Regions need recalculation */
    Boolean isDragging;             /* Currently being dragged */
    Boolean isResizing;             /* Currently being resized */
    Point dragOffset;               /* Offset during dragging */
} WindowExtension;

/* Window update queue entry */
typedef struct UpdateQueueEntry {
    WindowPtr window;
    WindowUpdateFlags flags;
    struct UpdateQueueEntry* next;
} UpdateQueueEntry;

/* Platform-specific data structure */
typedef struct PlatformData {
    void* nativeDisplay;            /* Native display connection */
    void* nativeScreen;             /* Native screen reference */
    unsigned long eventMask;        /* Event mask for native events */
    Boolean compositing;            /* Compositing window manager */
    Boolean accelerated;            /* Hardware accelerated */
} PlatformData;
...
typedef struct WindowManagerInternalState {
    /* Update management */
    UpdateQueueEntry* updateQueue;
    Boolean updatesEnabled;
    unsigned long lastUpdateTime;

    /* Tracking state */
    WindowPtr dragWindow;
    WindowPtr resizeWindow;
    Point lastMousePos;
