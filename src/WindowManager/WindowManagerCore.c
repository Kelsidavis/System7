...
static void CopyPascalString(const unsigned char* source, unsigned char* dest);
...
static void InitializeWMgrPort(void) {
    ...
    Platform_InitializePort(&g_wmState.wMgrPort->port);

    /* Set Window Manager specific fields */
    g_wmState.wMgrPort->windowList = NULL;
    g_wmState.wMgrPort->activeWindow = NULL;
    g_wmState.wMgrPort->ghostWindow = NULL;
    g_wmState.wMgrPort->menuBarHeight = 20; /* Standard Mac OS menu bar height */
    g_wmState.wMgrPort->initialized = true;

    /* Get screen bounds for gray region */
    Platform_GetScreenBounds(&g_wmState.wMgrPort->grayRgn);
...
static void InitializeDesktopPattern(void) {
    ...
    if (g_wmState.wMgrPort) {
        g_wmState.wMgrPort->deskPattern = g_wmState.desktopPattern;
    }
...
static void AddWindowToList(WindowPtr window, WindowPtr behind) {
    ...
    if (g_wmState.wMgrPort) {
        g_wmState.wMgrPort->windowList = g_wmState.windowList;
    }
...
static void RemoveWindowFromList(WindowPtr window) {
    ...
    if (g_wmState.wMgrPort) {
        g_wmState.wMgrPort->windowList = g_wmState.windowList;
        g_wmState.wMgrPort->activeWindow = g_wmState.activeWindow;
    }
