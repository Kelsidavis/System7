...
OSErr GetCharMetrics(short familyID, short size, short style, char character, CharMetrics *metrics)
{
    ...
    /* Set bounding box */
    metrics->boundingBox.left = 0;
    metrics->boundingBox.top = -(fontMetrics.ascent >> 16);
    metrics->boundingBox.right = metrics->width >> 16;
    metrics->boundingBox.bottom = fontMetrics.descent >> 16;
                         ...
                         short length, short measureMode, TextMetrics *metrics)
{
    ...
    metrics->boundingBox.left = 0;
    metrics->boundingBox.top = -(fontMetrics.ascent >> 16);
    metrics->boundingBox.right = totalWidth >> 16;
    metrics->boundingBox.bottom = fontMetrics.descent >> 16;
...
static OSErr GetFontMetricsFromBitmap(BitmapFontData *fontData, FontMetrics *metrics)
{
    ...
    metrics->ascent = (long)fontData->header.ascent << 16;
    metrics->descent = (long)fontData->header.descent << 16;
    metrics->leading = (long)fontData->header.leading << 16;
    metrics->lineHeight = metrics->ascent + metrics->descent + metrics->leading;
    metrics->widMax = (long)fontData->header.widMax << 16;
