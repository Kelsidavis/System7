        ...
        switch (itemHit) {
            case sfItemOpenButton:
                {
                    ...
                    if (selectedIndex >= 0 && selectedIndex < gSFState.numFiles) {
                        ...
                        cpb.hFileInfo.ioNamePtr = selected->name;
                        cpb.hFileInfo.ioVRefNum = selected->vRefNum;
                        cpb.hFileInfo.ioDirID = selected->parID;

                        err = PBGetCatInfoSync(&cpb);
                        if (err == noErr) {
                            if (cpb.hFileInfo.ioFlAttrib & 0x10) {
                                /* It's a folder - navigate into it */
                                gSFState.dirID = cpb.dirInfo.ioDrDirID;
                                SF_PopulateFileList();
                            } else {
                                /* It's a file - return it */
                                reply->sfFile = *selected;
                                reply->sfGood = true;
                                reply->sfType = cpb.hFileInfo.ioFlFndrInfo.fdType;
            ...
            default:
                ...
                if (itemHit >= 200 && itemHit < 200 + gSFState.numFiles) {
                    ...
                    err = PBGetCatInfoSync(&cpb);
                    if (err == noErr && (cpb.hFileInfo.ioFlAttrib & 0x10)) {
...
static void SF_PopulateFileList(void) {
    ...
    while (true) {
        memset(&cpb, 0, sizeof(cpb));
        cpb.hFileInfo.ioNamePtr = tempSpec.name;
        cpb.hFileInfo.ioVRefNum = gSFState.vRefNum;
        cpb.hFileInfo.ioFDirIndex = index;
        cpb.hFileInfo.ioDirID = gSFState.dirID;
        ...
        if (cpb.hFileInfo.ioFlAttrib & 0x10) {
            ...
            if (gSFState.fileFilter) {
                /* Use custom filter */
                include = !gSFState.fileFilter(&cpb, gSFState.userData);
        ...
        if (include && gSFState.numFiles < 256) {
            ...
            /* Add to dialog list */
            StandardFile_HAL_AddFileToList(gSFState.dialog, tempSpec.name,
                                          cpb.hFileInfo.ioFlAttrib & 0x10);
        }
