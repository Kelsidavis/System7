...
{
    OSErr err;
    uint8_t buffer[BLOCK_SIZE * 2];  /* MDB can span 2 blocks */
...
OSErr VCB_Unmount(VCB* vcb)
{
    ...
    if (g_FSGlobals.defVRefNum == vcb->vcbVRefNum) {
        g_FSGlobals.defVRefNum = g_FSGlobals.vcbQueue ?
                                  g_FSGlobals.vcbQueue->vcbVRefNum : 0;
    }
...
OSErr VCB_Flush(VCB* vcb)
{
    ...
    if (vcb->vcbVBMCache) {
        ...
        uint32_t bitmapBlocks = (vcb->vcbNmAlBlks + 4095) / 4096;
...
FCB* FCB_Alloc(void)
{
    ...
    /* Update free list */
    g_FSGlobals.fcbFree = (uint16_t)fcb->fcbRefNum;
...
void FCB_Free(FCB* fcb)
{
    ...
    /* Calculate index */
    index = (uint16_t)(fcb - g_FSGlobals.fcbArray);
...
OSErr FCB_Flush(FCB* fcb)
{
    ...
    fileRec.filClpSize = (uint16_t)fcb->fcbClmpSize;
