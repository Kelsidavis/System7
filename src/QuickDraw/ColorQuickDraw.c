...
static bool g_colorQDAvailable = false;
...
static PixMapHandle CreatePixMap(int16_t width, int16_t height, int16_t depth);
static CTabHandle CreateColorTable(int16_t numColors);
static void InitializeDevice(GDHandle device, int16_t depth);
static RGBColor IndexToRGB(int32_t index);
static int32_t RGBToIndex(const RGBColor *color);
...
void CopyPixMap(PixMapHandle srcPM, PixMapHandle dstPM) {
    ...
    *dst = *src;
    ...
    if (src->pmTable) {
        dst->pmTable = GetCTable((*src->pmTable)->ctSize + 1);
        if (dst->pmTable) {
            memcpy(*dst->pmTable, *src->pmTable,
                   sizeof(ColorTable) + (*src->pmTable)->ctSize * sizeof(ColorSpec));
        }
    }
}
...
PixPatHandle NewPixPat(void) {
    ...
    *pp = pixPat;
    ...
    memset(pixPat->pat1Data.pat, 0xFF, 8);
...
void MakeRGBPat(PixPatHandle pp, const RGBColor *myColor) {
    ...
    uint8_t gray = (uint8_t)((myColor->red + myColor->green + myColor->blue) / (3 * 256));
    memset(pixPat->pat1Data.pat, gray > 128 ? 0x00 : 0xFF, 8);
    ...
    *cTable = table;
    ...
    for (int i = 0; i < ctID; i++) {
        ...
        if (i == 0) {
            ...
            uint16_t gray = (uint16_t)(65535 * (ctID - 1 - i) / (ctID - 1));
...
static RGBColor IndexToRGB(int32_t index) {
    ...
    uint16_t gray = (uint16_t)((index & 0xFF) * 257);
