...
BlockDevice* MVFS_CreateATABlockDevice(int ata_device_index) {
    ...
    BlockDevice* dev = (BlockDevice*)NewPtr(sizeof(BlockDevice));
    if (!dev) {
        return NULL;
    }

    ATABlockDevPrivate* priv = (ATABlockDevPrivate*)NewPtr(sizeof(ATABlockDevPrivate));
    if (!priv) {
        DisposePtr((Ptr)dev);
        return NULL;
    }
...
BlockDevice* MVFS_CreateMemoryBlockDevice(void* buffer, size_t size) {
    ...
    MemoryBlockDevPrivate* priv = (MemoryBlockDevPrivate*)NewPtr(sizeof(MemoryBlockDevPrivate));
    if (!priv) {
        DisposePtr((Ptr)dev);
        return NULL;
    }
